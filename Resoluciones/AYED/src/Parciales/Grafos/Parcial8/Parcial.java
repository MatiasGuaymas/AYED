package Parciales.Grafos.Parcial8;
...
public class Parcial {
    public List<String> caminoConPresupuesto(Graph<String> ciudades, String origen, String destino, int montoMinimo) {
        ...
        if(!ciudades.isEmpty()) {
            ...
            if(v1 != null && v2 != null) {
                this.dfs(ciudades, v1, v2, camino, new boolean[ciudades.getSize()], montoMinimo, 0);
            }
        }
        return camino;
    }
    
    private boolean dfs(Graph<String> ciudades, Vertex<String> origen, Vertex<String> destino, List<String> lis, boolean[] marcas, int montoMinimo, int actual) {
        boolean encontre = false;
        marcas[origen.getPosition()] = true;
        lis.add(origen.getData());
        if(origen == destino && actual > montoMinimo) {
            return true;
        } else {
            ...
            while(it.hasNext() && !encontre) {
                ...
                if(!marcas[aux.getPosition()]) {
                    encontre = this.dfs(ciudades, aux, destino, lis, marcas, montoMinimo, actual + peso);
                }
            }
        }
